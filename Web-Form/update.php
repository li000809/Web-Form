...
if (isset($_POST["id"]) && !empty($_POST["id"])) {
    ...
    // $input_birth = trim($_POST['birth']);
    // // $birth_date = new DateTime($input_birth);
    // $birth_date = DateTime::createFromFormat('Y-m-d', $input_birth);
    // $today = new DateTime();
    // $interval = $today->diff($birth_date);
    // $age = $interval->y;
    // if ($age < 16) {
    ...
    // } else {
    //     $birth = $input_birth;
    //     echo 'Birth in update.php: ' . $birth . '<br>';
    // }
...
<!-- // $image_updated = false;
    ...
    // if (isset($_FILES['image']) && $_FILES['image']['error'] === UPLOAD_ERR_OK) {
    //     // Your existing image validation and upload code
    //     $image_updated = true;
    // } elseif ($_FILES['image']['error'] === UPLOAD_ERR_NO_FILE) {
    //     // No new image file is uploaded, keep the old image file name
    //     $image = $candidate->getImage();
    // } else {
    //     $image_err = "Error uploading the image. Please try again.";
    // }
